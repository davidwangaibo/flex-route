...
## 3. 整体流程（跟着 Flex 的序号走）
Flex App 在取货扫码时会弹出这件是整个 block 的第几站。程序直接记录这个序号，不再需要手写在箱子上：
```
取货：Flex 扫一下看到 4 ──▶ 本程序扫一下，键盘按 4，加入（地址可选）
装车：分拣页再扫一次 ──▶ 大字弹出 4，按号码摆放
送货：送达页按序号逐站「发送到 Tesla」
```
同一序号的多件包裹自动合并为一站。地址可以取货时拍面单 OCR，也可以之后在站点页「编辑」补上。
「自动优化顺序」仍保留，作为没有 Flex 序号时的备用。
