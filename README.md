...
## 3. 整体流程（方案 A：导入 Flex 路线，装车拍面单）
Flex 扫码取货是 Amazon 强制的，无法省略；但"每件包裹在两个 App 之间看数字再输入"可以完全去掉：
```
取货：Flex 扫码（必须）──▶ Flex 路线列表视图截屏 4~5 张 ──▶ 本程序一次导入（OCR 读出顺序 + 地址，可核对修改）
装车：本程序拍面单 ──▶ OCR 地址 ──▶ 自动匹配到第几站 ──▶ 大字显示，号码大的放最下面
        （拍过一次的包裹条码就认识了，再扫条码即可；识别不了时输门牌号前几位）
送货：送达页按顺序逐站「发送到 Tesla」
```
备用：「手动」页可以扫条码 + 手输序号；「自动优化顺序」可按直线距离自己排。
